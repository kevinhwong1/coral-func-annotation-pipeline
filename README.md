...
| *Xenia* spp. | Xspp | [NCBI GCF_021976095.1](https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_021976095.1/) (RefSeq mirror of Hu et al. 2020 Carnegie assembly, XeniaSp_v1) | — |
...
| *Xenia* spp. | Xspp | 26,116 | 25.4% | 29.0% | 11.1% | 0.3% | 34.2% | 73.8% | 12.1% | 14.1% | 2026-07-20 |
...
Eight jobs are submitted per species via LSF/BSUB. BSUB job scripts are generated at runtime by `submit_pipeline.sh` and written to the species run directory. All 10 project species are pre-configured in `run_config.sh`; new species can be run by passing paths directly as flags without editing any config file.
...
# 3. Submit pipeline (LSF/BSUB)
# Pre-configured species (Gfas, Pdam, Acer, Ofav, Amur, Amil, Spis, Opat, Xspp, Nvec):
bash pipeline/submit_pipeline.sh <SPECIES>
# e.g. bash pipeline/submit_pipeline.sh Acer

# New species not in run_config.sh — pass paths directly:
bash pipeline/submit_pipeline.sh <SPECIES> \
